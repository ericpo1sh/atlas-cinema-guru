        ...
        return (
          ...
          >
            ...
            <div className="absolute top-3 right-3 flex gap-3 opacity-0 group-hover:opacity-100 transition-opacity z-20">
              {movie.favorited ? (
                <StarIcon className="w-8 h-8 text-yellow-400 cursor-pointer" onClick={handleFavoriteToggle} />
              ) : (
                <StarOutlineIcon className="w-8 h-8 text-white cursor-pointer" onClick={handleFavoriteToggle} />
              )}
              {movie.watchLater ? (
                <TimeIcon className="w-8 h-8 text-blue-400 cursor-pointer" onClick={handleWatchLaterToggle} />
              ) : (
                <TimeOutlineIcon className="w-8 h-8 text-white cursor-pointer" onClick={handleWatchLaterToggle} />
              )}
            </div>
            <div
